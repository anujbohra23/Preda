...
class Session(db.Model):
    ...
    intake_fields   = db.relationship('IntakeField',   backref='session', lazy='dynamic',
                                      cascade='all, delete-orphan')
    uploads         = db.relationship('Upload',        backref='session', lazy='dynamic',
                                      cascade='all, delete-orphan')
    disease_results = db.relationship('DiseaseResult', backref='session', lazy='dynamic',
                                      cascade='all, delete-orphan')
...
class Upload(db.Model):
    ...
    chunks = db.relationship('ExtractedChunk', backref='upload', lazy='dynamic',
                             cascade='all, delete-orphan')
...
class ExtractedChunk(db.Model):
    ...
    rag_retrievals = db.relationship('RagRetrieval', backref='chunk', lazy='dynamic',
                                     cascade='all, delete-orphan')
...
class DiseaseResult(db.Model):
    ...
    id               = db.Column(db.Integer, primary_key=True)
    session_id       = db.Column(db.Integer, db.ForeignKey('sessions.id'), nullable=False, index=True)
    disease_id       = db.Column(db.Integer, db.ForeignKey('disease_catalog.id'), nullable=False)
...
class ChatMessage(db.Model):
    ...
    rag_retrievals = db.relationship('RagRetrieval', backref='chat_message', lazy='dynamic',
                                     cascade='all, delete-orphan')
...
class RagRetrieval(db.Model):
    ...
    chunk_id         = db.Column(db.Integer, db.ForeignKey('extracted_chunks.id'), nullable=False)
    similarity_score = db.Column(db.Float, nullable=False)
