...
from ..models import (
    User, Session, IntakeField, Upload,
    ExtractedChunk, DiseaseResult, ChatMessage,
    RagRetrieval, Report, AuditLog
)
...
def get_pharmacy_settings(user_id: int) -> dict:
    row = (
        AuditLog.query
        .filter_by(user_id=user_id, event_type='pharmacy_settings_saved')
...
def delete_account():
    ...
    for s in sessions:
        ...
        msg_ids = [
            m.id for m in
            ChatMessage.query.filter_by(session_id=s.id).all()
        ]
        if msg_ids:
            RagRetrieval.query.filter(
        ...
        chunk_ids = [
            c.id for c in
            ExtractedChunk.query.filter_by(session_id=s.id).all()
        ]
