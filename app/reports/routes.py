...
from ..models import (
    Session, IntakeField, DiseaseResult,
    ChatMessage, RagRetrieval, Report, AuditLog
)
from .generator import (
    build_patient_report, build_pharmacy_report,
    render_report_pdf, save_pdf
)
...
def _gather_session_data(s: Session) -> dict:
    ...
    if msg_ids:
        retrievals = (
            RagRetrieval.query
            .filter(RagRetrieval.chat_message_id.in_(msg_ids))
            .all()
        )
    return {
        'intake':         intake,
...
def report_page(session_id):
    ...
    patient_report  = Report.query.filter_by(
        session_id=session_id, report_type='patient'
    ).order_by(Report.generated_at.desc()).first()

    pharmacy_report = Report.query.filter_by(
        session_id=session_id, report_type='pharmacy'
    ).order_by(Report.generated_at.desc()).first()
...
def generate_report(session_id):
    ...
    if not data['diseases']:
        flash(
            'Please complete condition matching before generating a report.',
            'warning'
        )
    ...
    try:
        ...
        stored_path = save_pdf(
            pdf_bytes, session_id, report_type, upload_folder
        )
    ...
    _log_audit(current_user.id, session_id, 'report_generated', {
        'report_type': report_type,
    })
    db.session.commit()
...
def download_report(report_id):
    ...
    if not report.pdf_path or not os.path.exists(report.pdf_path):
        ...
        return redirect(
            url_for('reports.report_page', session_id=report.session_id)
        )

    _log_audit(current_user.id, report.session_id, 'pdf_exported', {
        'report_id':   report.id,
