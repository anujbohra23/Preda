...
from ..models import Report, Session, AuditLog
from ..email.mailer import send_pharmacy_report, is_configured
...
def consent_page(report_id):
    ...
    if session.safety_flagged:
        flash(
            'Email sharing is disabled for sessions with safety alerts.',
            'error'
        )
        return redirect(
            url_for('reports.report_page', session_id=report.session_id)
    ...
    if not pharmacy.get('pharmacy_email'):
        flash(
            'Please save your pharmacy details in Settings first.',
            'warning'
        )
    ...
    if form.validate_on_submit():
        ...
        _log_audit(current_user.id, report.session_id,
                   'email_consent_given', {
                       'report_id':       report.id,
