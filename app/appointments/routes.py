...
from ..models import Session, Appointment, AppointmentAction, AuditLog
from .forms import AudioUploadForm, ManualNotesForm, ConsentForm
from .transcriber import transcribe_audio, whisper_available
from .summariser import summarise, extract_actions
...
def _audio_upload_dir(session_id: int) -> str:
    ...
    d    = os.path.join(base, 'appointments',
...
def _process_and_save(appt: Appointment, text: str):
    ...
    # Save actions
    AppointmentAction.query.filter_by(
        appointment_id=appt.id
    ).delete()

    for action_data in extract_actions(summary):
        db.session.add(AppointmentAction(
            appointment_id = appt.id,
...
@appointments_bp.route('/session/<int:session_id>/record',
                        methods=['GET', 'POST'])
@login_required
def record_appointment(session_id):
    ...
    if request.method == 'GET':
        return render_template(
            'appointments/record.html', session=s, form=form
        )
    ...
    if not t_result['success']:
        ...
        return redirect(
            url_for('appointments.detail', appt_id=appt.id)
        )
    ...
    _log_audit(session_id, 'appointment_created', {
        'appt_id': appt.id, 'method': 'recording'
    })
    db.session.commit()
...
@appointments_bp.route('/session/<int:session_id>/upload',
                        methods=['GET', 'POST'])
@login_required
def upload_appointment(session_id):
    ...
    if form.validate_on_submit():
        ...
        if not audio_file:
            ...
            return render_template(
                'appointments/upload.html', session=s, form=form
            )
...
@appointments_bp.route('/session/<int:session_id>/manual',
                        methods=['GET', 'POST'])
@login_required
def manual_appointment(session_id):
    ...
    return render_template(
        'appointments/manual.html', session=s, form=form
    )
...
def toggle_action(action_id):
    ...
    if action is None:
        abort(404)
    appt = _own_appointment_or_404(action.appointment_id)
...
def delete_appointment(appt_id):
    ...
    return redirect(
        url_for('appointments.list_appointments', session_id=session_id)
    )