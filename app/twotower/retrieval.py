...
def build_query_text(intake_fields: dict, confirmed_chunks: list[str]) -> str:
    parts = []
    field_order = [
        "chief_complaint",
        "duration",
...
def explain_match(
    disease: dict, intake_fields: dict, confirmed_chunks: list[str]
) -> dict:
    """
    Explainability via:
    ...
    field_contributions = {
        ...
        for k, v in sorted(field_scores.items(), key=lambda x: x[1], reverse=True)
    }
